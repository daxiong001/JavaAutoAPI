...
public class Reporter implements IReporter {
	...
	private String generateHtml() {
		...
		if (results.size() > 1) {
			...
			.append("<td>").append(skippedNum).append("</td>\n")
		...
		for (Suite suite : results) {
			...
			tableOfContents.append("<h2>").append("Test Suite : " + suite.getName()).append("</h2>\n")			
					.append("<table class=\"table table-striped table-bordered\">\n")
					.append("<h3>").append("Summary Reporter").append("</h3>\n")
					.append("<thead>\n")
					.append("<tr>\n").append("<th>").append("# Total").append("</th>\n")
					.append("<th>").append("# Passed").append("</th>\n")
					.append("<th>").append("# Failed").append("</th>\n")
					.append("<th>").append("# Skipped").append("</th>\n")
					.append("<th>").append("# Passed Rate").append("</th>\n")
					.append("<th>").append("# Start Time").append("</th>\n")
					.append("<th>").append("# End Time").append("</th>\n").append("</tr>\n").append("</thead>\n")
					.append("<tbody>\n").append("<tr>\n").append("<td>").append(suite.getSum()).append("</td>\n")
					.append("<td>").append(suite.getPassedNum()).append("</td>\n")
					.append("<td>").append(suite.getFailedNum()).append("</td>\n")
					.append("<td>").append(suite.getSkippedNum()).append("</td>\n")
					.append("<td>").append(ReporterUtil.rateConvertor(suite.getPassedRate())).append("</td>\n")
					.append("<td>").append(DateUtil.parseDateToString(suite.getStartTime(), DateUtil.FORMAT7)).append("</td>\n")
					.append("<td>").append(DateUtil.parseDateToString(suite.getEndTime(), DateUtil.FORMAT7)).append("</td>\n").append("</tr>\n").append("</tbody>\n\n\n");
			
			tableOfContents
			.append("<table class=\"table table-striped table-bordered\">\n")
			.append("<h3>").append("Detail Result").append("</h3>\n")
			.append("<thead>\n")
			.append("<tr>\n").append("<th>").append("# Class Name").append("</th>\n")
			.append("<th>").append("# Method Name").append("</th>\n")
			.append("<th>").append("# Status").append("</th>\n")
			.append("<th>").append("# Error").append("</th>\n").append("</tr>\n").append("</thead>\n")
			.append("<tbody>\n");

			if (passedCases != null || passedCases.size() > 0) {
				for (TestCase ts : passedCases) {
					tableOfContents.append("<tr  class=\"success\">\n")
					.append("<td>").append(ts.getClassName()).append("</td>\n")
